...
The following will create a component where you are going to use as your homepage
...
The script created a component folder `./angular-application/src/app/high-scores` containing 4 files:
...
### 2. Set component as homepage in the Angular Routing Module
...
The `HighScoresComponent` component is now the homepage of your Angular application
...
For this chapter you will also use the component you created in the previous chapter: `./angular-application/src/app/high-scores/*`

### 1. Create the component

Create a (sub) component inside `./angular-application/src/app/high-scores` component directory.

```sh
ng generate component high-scores/high-scores-list --module=app
```

The script created `./angular-application/src/app/high-scores/high-scores-list/*` component.
...
### 3. Have the high scores list component take high scores as @Input

Place the following in file: `./angular-application/src/app/high-scores/high-scores-list/app-high-scores-list.ts`
...
`@Input` is used to have other components pass data inside components.

`@Output`  makes you able to publish events to the component using this component

### 4. Display high scores in a list using Material components
...
It will currently show an empty list when running the application. In the next chapter you will provide this component with actual high scores.
...
## Connect the high scores and high scores list components
...
### 2. Pass the highscores array to the app-high-scores-list component

Replace the content in the following file: `./angular-application/src/app/high-scores/high-scores.component.html`
...
> #### Suggestion
> 
> Run the application and validate if you see the high score being logged to the console when you click on it.
...
The following component is generated:  `./angular-application/src/app/add-score/*`
...
Create a new form with Material components in the new add score component `./angular-application/src/app/add-score.component.scss`
...
Let's add some style to the component for better spacing `./angular-application/src/app/add-score/add-score.component.scss`
...
Create the form using Angular `FormBuilder` in file `./angular-application/src/app/add-score.component.ts`
...
Also make the following changes to the html in file `./angular-application/src/app/add-score.component.html` to attach the `addScoreForm`. The properties in the created `addScoreForm` map to the `formControlName` in the html file. 
